function [ kappa_eff, mu_eff] = dilute_analy( ci, kappa_m, mu_m, kappa_i, mu_i, str)
...
switch str
    case 'iso'
        ...
        kappa_eff = kappa_m + ci * ( kappa_i - kappa_m) * kappa_m / ( kappa_m + alpha * ( kappa_i - kappa_m));
        mu_eff = mu_m + ci * ( mu_i - mu_m) * mu_m / ( mu_m + beta * ( mu_i - mu_m));
    case 'plane strain'
        ...
        C_eff = C_m + ci * ( C_i - C_m) * inv( eye(3) + S_m * inv( C_m) * ( C_i - C_m));
